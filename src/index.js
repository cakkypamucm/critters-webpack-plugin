...
import { tap, createLogger } from "./util.js";
...
export default class CrittersWebpackPlugin extends Critters {
    ...
    async getCssAsset(href, style) {
        ...
        if (!sheet) {
            try {
                ...
                this.logger.warn(
                    `Critters: stylesheet "${relativePath}" not found in assets, but a file was located on disk.${
                        this.options.pruneSource ? " This means pruneSource will not be applied." : ""
                    }`
                );
            } catch (e) {
                this.logger.warn(`Critters: unable to locate stylesheet: ${relativePath}`);
                return;
            }
        }
    ...
    checkInlineThreshold(link, style, sheet) {
        ...
        if (inlined) {
            ...
            if (asset) {
                ...
                this.logger.warn(
                    `Critters: > ${style.$$name} was not found in assets. the resource may still be emitted but will be unreferenced.`
                );
            }
        }
    ...
    async embedAdditionalStylesheet(document) {
        ...
        (this.options.additionalStylesheets || []).forEach((cssFile) => {
            ...
            const webpackCssAssets = Object.keys(this.compilation.assets).filter((file) => minimatch(file, cssFile));
    ...
    pruneSource(style, before, sheetInverse) {
        ...
        if (asset) {
            ...
            this.compilation.assets[style.$$assetName] = new sources.LineToLineMappedSource(
                sheetInverse,
                style.$$assetName,
                before
            );
            ...
            this.logger.warn(
                "Critters: pruneSource is enabled, but a style (" + name + ") has no corresponding Webpack asset."
            );
        }
